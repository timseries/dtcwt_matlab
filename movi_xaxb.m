function movi_xaxb(seq1,seq2,type,rate,minmax)

% function movi16(seq1,seq2,type,rate,minmax)
% 
% Display movies of two 3-D uint16, uint8 or double dataset seqs at max frame
% rate 'rate' fr/s (the actual rate can be lower due limited processor speed).
% seq1 / seq2 is a 3-D uint16 or uint8 array of frames seq(:,:,k), note
% that seq1 and seq2 are assumed to be of the same size in all 3 dimensions.
...
%   Dataset1/Dataset2 button toggles the displayed datasets.
%
...
%
% Nick Kingsbury, Cambridge University, Jan 2009.

global ind di brk newsel 

if nargin < 2, error('At least 2 datasets are required for comparison'); end
...
ui(13) = uicontrol(gcf,'style','toggle','pos',[450 0 40 20],'str','X-hairs','value',1);
...
% Toggle between seq1 and seq2
ui(17) = uicontrol(gcf,'style','toggle','pos',[490 0 50 20],'str','Dataset1','value',1);
...
im1 = uint8(double(seq1(:,:,ind(3)))*imscale+imoffset);
im2 = uint8(double(squeeze(seq1(:,ind(2),:)))*imscale+imoffset);
im3 = uint8(double(squeeze(seq1(ind(1),:,:)).')*imscale+imoffset);
...
dataset_sel = get(ui(17),'value'); % select dataset as seq1 or seq2
prev = [ind sel imscale imoffset xhairs dataset_sel]*0;
...
while 1,
   ...
   dataset_sel = get(ui(17),'value');
   
   % Update appropriate views if any parameters have changed.
   change = [ind sel imscale imoffset xhairs dataset_sel];
   ch = (change ~= prev); 
   if any(ch),
       if dataset_sel == 1 % If the user selects seq1
          set(ui(17),'str','Dataset1');
          % Only update one view if possible, for max speed.
          if any(ch & [0 0 1 0 1 1 0 1]),
             im1 = uint8(double(seq1(:,:,ind(3)))*imscale+imoffset); end
          if any(ch & [0 1 0 0 1 1 0 1]),
             im2 = uint8(double(squeeze(seq1(:,ind(2),:)))*imscale+imoffset); end
          if any(ch & [1 0 0 0 1 1 0 1]),
             im3 = uint8(double(squeeze(seq1(ind(1),:,:)).')*imscale+imoffset); end
       ...
       elseif dataset_sel == 0 % If the user selects seq2
          set(ui(17),'str','Dataset2'); 
          % Only update one view if possible, for max speed.
          if any(ch & [0 0 1 0 1 1 0 1]),
             im1 = uint8(double(seq2(:,:,ind(3)))*imscale+imoffset); end
          if any(ch & [0 1 0 0 1 1 0 1]),
             im2 = uint8(double(squeeze(seq2(:,ind(2),:)))*imscale+imoffset); end
          if any(ch & [1 0 0 0 1 1 0 1]),
             im3 = uint8(double(squeeze(seq2(ind(1),:,:)).')*imscale+imoffset); end
          set(imh,'CData',[im1 w1 im2; w2; im3 w3]);
          % Image overwrites cross-hairs, so rewrite these if required.
          if (xhairs),
             r=rand(4,1)*1e-6; % This ensures that all cross hairs are changed slightly and hence updated.
