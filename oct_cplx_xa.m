...
% within each frame of xa, and adjacent frames (along 3rd dim) are the real
...
for band = 1:7
    for frame = t3
        f2 = frame + [0 1];
        y1 = squeeze(ya{level}{band}(s1,s2,f2(1)));
        y2 = squeeze(ya{level}{band}(s1,s2,f2(2)));
        ...
        ya{level}{band}(s1,s2,f2(1)) = [a-d-f-g  a+d+f-g; a+d-f+g  a-d+f+g]*0.5;
        % Form the 4 subbands of imag parts.
        ya{level}{band}(s1,s2,f2(2)) = [b+c+e-h  -b+c+e+h; b-c+e+h  -b-c+e-h]*0.5;
    end
end
return
...
global ya
sya = size(ya{abs(level)}{1});
t3 = 1:2:sya(3);
s1 = 1:sya(1); s2 = 1:sya(2);
for band = 1:7
    for frame = t3
        f2 = frame + [0 1];
        y1 = squeeze(ya{abs(level)}{band}(s1,s2,f2(1)));  % Real
        y2 = squeeze(ya{abs(level)}{band}(s1,s2,f2(2)));  % Imag
        ...
        ya{abs(level)}{band}(t1,t2,f2(1))     =  (A+B+C+D)*0.5; % a 
        ya{abs(level)}{band}(t1,t2+1,f2(1))   =  (E-F+G-H)*0.5; % b 
        ya{abs(level)}{band}(t1+1,t2,f2(1))   =  (E+F-G-H)*0.5; % c
        ya{abs(level)}{band}(t1+1,t2+1,f2(1)) = (-A+B+C-D)*0.5; % d

        ya{abs(level)}{band}(t1,t2,f2(2))     =  (E+F+G+H)*0.5; % e 
        ya{abs(level)}{band}(t1,t2+1,f2(2))   = (-A+B-C+D)*0.5; % f 
        ya{abs(level)}{band}(t1+1,t2,f2(2))   = (-A-B+C+D)*0.5; % g
        ya{abs(level)}{band}(t1+1,t2+1,f2(2)) = (-E+F+G-H)*0.5; % h
    end
end
return